...
def add_BF_feature2(pns_df, bloom, hashed_account_dict1, hashed_account_dict2):
	"""add bloom filter feature to dataframe"""

	pns_df['Hash2'] = pns_df['BeneficiaryAccount'].astype(str) + pns_df['BeneficiaryName'].astype(str) + \
	                    pns_df[
		'BeneficiaryStreet'].astype(str) + pns_df['BeneficiaryCountryCityZip'].astype(str)

	pns_df['Hash1'] = pns_df['OrderingAccount'].astype(str) + pns_df['OrderingName'].astype(str) + pns_df[
		'OrderingStreet'].astype(str) + pns_df['OrderingCountryCityZip'].astype(str)

	pns_df['BF'] = pns_df.apply(
