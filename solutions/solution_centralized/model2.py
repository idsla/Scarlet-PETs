...
def generate_feature(df, pivot_name, new_feature_name, func, count_columns, mean_columns, agg_col=None, epsilon = 0.25, dp_flag = False):
    if func == 'value_count':
        ...
        if dp_flag:  # if use DP
            if new_feature_name in count_columns: # add laplace noise with sensitivity = 1
                ...
                df = df.drop(new_feature_name,axis=1).merge(d_dp, left_on=pivot_name, right_index=True) #replace with the dp version
            else:
                pass
